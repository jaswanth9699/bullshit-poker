...
test("room HTTP routes create, join, and start a room", async () => {
  ...
  const createResponse = await handleRoomHttpRequest(
    jsonRequest("/room/create", { roomId: "room-1", code: "ABC123", hostName: "Host", pin: "1234" }),
    context
  );
  ...
  const joinResponse = await handleRoomHttpRequest(jsonRequest("/room/join", { name: "Friend", pin: "5678" }), context);
  ...
  const startResponse = await handleRoomHttpRequest(jsonRequest("/room/start", { hostPlayerId: "player-1" }), context);
...
test("room HTTP routes reject bad method, bad JSON, and missing fields", async () => {
  ...
  const methodResponse = await handleRoomHttpRequest(new Request("https://example.test/room/create", { method: "GET" }), context);
  ...
  const jsonResponse = await handleRoomHttpRequest(
    new Request("https://example.test/room/create", { method: "POST", body: "not-json" }),
    context
  );
  assert.equal(jsonResponse.status, 400);
  assert.equal((await responseJson(jsonResponse)).code, "INVALID_JSON");

  const missingFieldResponse = await handleRoomHttpRequest(jsonRequest("/room/create", { roomId: "room-1" }), context);
  assert.equal(missingFieldResponse.status, 400);
  assert.equal((await responseJson(missingFieldResponse)).code, "INVALID_CREATE_ROOM_REQUEST");
});
...
test("room HTTP action route forwards submit claim envelope", async () => {
  ...
  await handleRoomHttpRequest(jsonRequest("/room/join", { name: "Friend", pin: "5678" }), context);
  const startResponse = await handleRoomHttpRequest(jsonRequest("/room/start", { hostPlayerId: "player-1" }), context);
  const startBody = await responseJson(startResponse);
  ...
  assert.equal(((submitBody.view as Record<string, unknown>).currentClaim as Record<string, unknown>).playerId, "player-1");
...
test("room HTTP routes add a bot in the lobby", async () => {
  ...
  const response = await handleRoomHttpRequest(
    jsonRequest("/room/bots/add", { hostPlayerId: "player-1", name: "Table Bot" }),
    context
  );
...
test("room HTTP routes remove a bot in the lobby", async () => {
  ...
  const bot = (addedView.players as Array<Record<string, unknown>>).find((player) => player.isBot);

  const removeResponse = await handleRoomHttpRequest(
    jsonRequest("/room/bots/remove", { hostPlayerId: "player-1", botPlayerId: bot?.id }),
    context
  );
...
test("room HTTP add-bot route validates host player id", async () => {
  ...
  const response = await handleRoomHttpRequest(jsonRequest("/room/bots/add", {}), context);
...
test("room HTTP routes advance from resolved round into the next round", async () => {
  ...
  await handleRoomHttpRequest(jsonRequest("/room/join", { name: "Friend", pin: "5678" }), context);
  const startResponse = await handleRoomHttpRequest(jsonRequest("/room/start", { hostPlayerId: "player-1" }), context);
  const startState = (await responseJson(startResponse)).view as Record<string, unknown>;
  ...
  const claimState = (await responseJson(submitResponse)).view as Record<string, unknown>;
  ...
  await handleRoomHttpRequest(
    jsonRequest("/room/actions/call-bullshit", {
      envelope: {
        ...
        claimWindowId: (claimState.activeClaimWindow as Record<string, unknown>).id,
        payload: {}
      }
    }),
    context
  ...
  const nextRoundResponse = await handleRoomHttpRequest(jsonRequest("/room/next-round", {}), context);
