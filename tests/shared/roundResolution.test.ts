...
function player(id: string, seatIndex: number, roundCards: PlayerState["roundCards"], cardCount = 1): PlayerState {
  return {
    id,
    name: id,
    normalizedName: id,
...
function baseState(overrides: Partial<GameState> = {}): GameState {
  ...
  return {
    ...
    currentClaim: { id: "claim-1", handType: "PAIR", primaryRank: "K", playerId: "B", sequence: 1 },
    activeClaimWindow: {
      id: "window-1",
      claimId: "claim-1",
      roundNumber: 3,
...
test("false final claim penalizes final claimant and reveals cards", () => {
  ...
  const result = resolveFinalClaim(
    state,
    { id: "final-1", handType: "PAIR", primaryRank: "A", playerId: "A", sequence: 2 },
    500
  );
  ...
  assert.equal(result.state.players.find((candidate) => candidate.id === "A")?.cardCount, 2);
...
test("final claim must be strictly higher than current claim", () => {
  ...
  assert.throws(
    () => resolveFinalClaim(state, { id: "final-1", handType: "PAIR", primaryRank: "Q", playerId: "A" }, 500),
    /strictly higher/
  );
});
...
test("incorrect BullShit call penalizes caller when claim is true", () => {
  ...
  assert.equal(result.state.players.find((candidate) => candidate.id === "C")?.cardCount, 2);
});
...
test("correct BullShit call penalizes claimant when claim is false", () => {
  ...
  assert.equal(result.state.players.find((candidate) => candidate.id === "B")?.cardCount, 2);
});

test("timeout penalizes current player and can eliminate them", () => {
  const state = baseState({
    currentTurnPlayerId: "B",
    players: [player("A", 0, [createCard("A", "S")]), player("B", 1, [createCard("K", "H")], 5), player("C", 2, [createCard("2", "D")])]
  });
  ...
  assert.equal(result.roundResult.penaltyPlayerId, "B");
  assert.equal(result.state.players.find((candidate) => candidate.id === "B")?.cardCount, 6);
  assert.equal(result.state.players.find((candidate) => candidate.id === "B")?.eliminated, true);
